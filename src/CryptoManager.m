...
@property(strong)	RSAPublicKey*	publicKey2048;
@property(strong)	RSAPublicKey*	publicKey1024;
...
- (BOOL)setupKeyPairForSizeInBits:(NSInteger)bitSize;
{
	...
	@try {
		...
		if (ret != errSecSuccess) {
			...
			CFRelease(returnRef);	// privateKeyに代入されないので@catchでは解放されない
		...
		if (!convertedKey) {
			...
			case 1024:
				...
				break;
			default:
				// Release resources before raising exception to prevent leak
				if (privateKey) {
		...
		case 2048:
			...
			if (_privateKey2048) { CFRelease(_privateKey2048); }
			self.privateKey2048	= privateKey;
			privateKey = NULL;	// 所有権移譲済み、@catchでの解放を防ぐ
			break;
		case 1024:
			self.publicKey1024	= convertedKey;
			if (_privateKey1024) { CFRelease(_privateKey1024); }
			self.privateKey1024	= privateKey;
			privateKey = NULL;	// 所有権移譲済み、@catchでの解放を防ぐ
			break;
		default:
			// Release privateKey before raising exception to prevent leak
			if (privateKey) {
				CFRelease(privateKey);
				privateKey = NULL;	// @catchでの二重解放を防ぐ
			}
			// Release publicKey if not yet released
			if (publicKey) {
				CFRelease(publicKey);
				publicKey = NULL;
			}
	...
	@catch (NSException* exp) {
		...
		// privateKeyがNULLでなければ解放（所有権移譲済みの場合はNULL）
		if (privateKey) {
			CFRelease(privateKey);
		}
		return NO;
