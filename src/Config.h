...
@property(strong)	NSFont*				sendMessageFont;			// 送信ウィンドウメッセージ部フォント
@property(readonly)	NSFont*				defaultSendMessageFont;		// 送信ウィンドウメッセージ標準フォント
// 受信
@property(readonly)	NSSound*			receiveSound;				// 受信音
@property(copy, nullable)	NSString*	receiveSoundName;			// 受信音名
...
@property(strong)	NSFont*				receiveMessageFont;			// 受信ウィンドウメッセージ部フォント
...
// ----- getter / setter ------
// ネットワーク
- (nullable NSString*)broadcastAtIndex:(NSUInteger)index;
...
// 不在
- (nullable NSString*)absenceTitleAtIndex:(NSUInteger)index;
- (nullable NSString*)absenceMessageAtIndex:(NSUInteger)index;
...
// 通知拒否
- (nullable RefuseInfo*)refuseInfoAtIndex:(NSUInteger)index;
