...
{
	return [[RSAPublicKey alloc] initWithNativeKey:key exponent:exp modulus:mod];
}

// ファクトリ（Exp, Mod）
+ (instancetype)keyWithExponent:(UInt32)exp modulus:(NSData*)mod
{
	return [[RSAPublicKey alloc] initWithExponent:exp modulus:mod];
}

// ファクトリ（SecKeyRef）
+ (instancetype)keyWithNativeKey:(SecKeyRef)key
{
	return [[RSAPublicKey alloc] initWithNativeKey:key];
}
...
- (instancetype)initWithNativeKey:(SecKeyRef)key
{
	...
	if (self) {
		...
		@try {
			...
			NSData* derData = (__bridge_transfer NSData*)derRef;
