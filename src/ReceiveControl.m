...
- (instancetype)initWithRecvMessage:(RecvMessage*)msg
{
	...
	if (self) {
		...
		for (RecvClipboard* clip in _recvMsg.clipboards) {
			...
			NSUInteger insertPos = MIN(clip.clipboardPos, maxLength);
			if (insertPos != clip.clipboardPos) {
				WRN(@"Clipboard position out of range (pos=%lu, max=%lu), adjusted to %lu",
					(unsigned long)clip.clipboardPos, (unsigned long)maxLength, (unsigned long)insertPos);
			}
