...
static NSMutableArray*	gOpenRecvWindows = nil;		// 開いている受信ウィンドウの強参照(ARC自己保持用)
...
@property(strong)	NSDate*					dlStart;				// ダウンロード開始時刻
@property(weak)		id<DownloaderContext>	download;				// ダウンロード情報
@property(strong)	NSTimer*				dlSheetRefreshTimer;	// ダウンロードシート更新タイマ
...
- (instancetype)initWithRecvMessage:(RecvMessage*)msg
{
	...
	if (self) {
		...
		for (RecvClipboard* clip in _recvMsg.clipboards) {
			NSTextAttachmentCell*	cell	= [[NSTextAttachmentCell alloc] initImageCell:clip.image];
			NSTextAttachment*		attach	= [[NSTextAttachment alloc] init];
			attach.attachmentCell = cell;
			NSAttributedString* str = [NSAttributedString attributedStringWithAttachment:attach];
			// 範囲外アクセスを防ぐため、挿入位置をテキストストレージの長さ以内に制限
			NSUInteger maxLength = _messageArea.textStorage.length;
			NSUInteger insertPos;
			
			// macOS から送信された場合 (clipboardPos == 0) など、位置情報がないか0の場合は末尾に挿入する
			if (clip.clipboardPos == 0) {
				insertPos = maxLength;
			} else {
				insertPos = MIN(clip.clipboardPos, maxLength);
			}
			
			// 末尾に挿入し、メッセージの最後が改行でない場合は改行を挟んでから画像を挿入して綺麗に見せる
			if (insertPos == maxLength && maxLength > 0) {
				NSString* lastChar = [_messageArea.textStorage.string substringWithRange:NSMakeRange(maxLength - 1, 1)];
				if (![lastChar isEqualToString:@"\n"]) {
					NSAttributedString* newline = [[NSAttributedString alloc] initWithString:@"\n"];
					[_messageArea.textStorage insertAttributedString:newline atIndex:maxLength];
					maxLength += 1;
					insertPos = maxLength;
				}
			}
			
			[_messageArea.textStorage insertAttributedString:str atIndex:insertPos];
		}

		// ARC自己保持：ウィンドウが閉じるまでselfを生存させる
		@synchronized([ReceiveControl class]) {
			if (!gOpenRecvWindows) {
				gOpenRecvWindows = [[NSMutableArray alloc] init];
			}
			[gOpenRecvWindows addObject:self];
		}
	}
...
{
	// タイマーが有効であれば無効化
	if (_dlSheetRefreshTimer.isValid) {
		[_dlSheetRefreshTimer invalidate];
	}
}
...
- (IBAction)replyMessage:(id)sender
{
	...
	// 送信ダイアログ作成
	(void)[[SendControl alloc] initWithSendMessage:quotMsg recvMessage:self.recvMsg];
}
			...
			row:(NSInteger)rowIndex
{
	if (aTableView == self.attachTable) {
		...
		NSMutableAttributedString* cellValue = [[NSMutableAttributedString alloc] initWithString:attach.name];
...
- (void)windowWillClose:(NSNotification*)aNotification
{
	...
	@synchronized([ReceiveControl class]) {
		[gOpenRecvWindows removeObject:self];
	}
}
