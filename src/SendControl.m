...
static NSMutableArray*		gOpenSendWindows		= nil;	// 開いている送信ウィンドウの強参照(ARC自己保持用)
...
@property(strong)	_UserList*		users;						// ユーザ一覧
@property(strong)	NSPredicate*	userPredicate;				// ユーザ検索フィルタ
@property(strong)	_UserList*		selectedUsers;				// 選択ユーザリスト TODO:不要かも
@property(strong)	_AttachList*	attachments;				// 添付ファイル一覧
@property(strong)	_IconList*		icons;						// アイコン一覧
@property(strong)	id<NSObject>	userListChangedObserver;	// 通知オブサーバ
						...
						recvMessage:(RecvMessage*)recv
{
	...
	if (self) {
		...
		// ユーザリスト変更の通知登録（先に登録してから初期表示することで通知見逃しを防ぐ）
		...
		_userListChangedObserver = [nc addObserverForName:kIPMsgUserListChangedNotification
												...
												usingBlock:^(NSNotification* _Nonnull note) {
															...
															// 非同期でメインスレッドで処理する
															//TODO: メソッド側で必要な部分だけメインスレッドで処理するように変更
															dispatch_async(dispatch_get_main_queue(), ^{
																[strongSelf userListChanged:note];
															});
														}];

		// ユーザ数ラベルの設定（通知登録後に呼ぶことで、登録前にUDPスレッドが更新した分も反映）
		[self userListChanged:nil];

		// ARCのstrong propertyがウィンドウを管理するため、自動releaseを無効化（二重解放防止）
		...
		// ARC自己保持：ウィンドウが閉じるまでselfを生存させる
		@synchronized([SendControl class]) {
			if (!gOpenSendWindows) {
				gOpenSendWindows = [[NSMutableArray alloc] init];
			}
			[gOpenSendWindows addObject:self];
		}
	}
...
- (IBAction)checkboxChanged:(id)sender
{
	...
	if (sender == self.sealCheck) {
		...
		if (!state) {
            self.passwordCheck.state = NSControlStateValueOff;
		}
	}
	// 鍵チェックボックス
...
- (void)appendAttachmentByPath:(NSString*)path
{
	...
	if (newIcon) {
		...
		newIcon = [[NSImage alloc] initWithSize:NSMakeSize(16,16)];
	}
...
- (IBAction)searchMenuItemSelected:(id)sender
{
	if ([sender isKindOfClass:NSMenuItem.class]) {
        ...
        NSControlStateValue	newSt	= (item.state == NSControlStateValueOn) ? NSControlStateValueOff : NSControlStateValueOn;
        BOOL				newVal	= (BOOL)(newSt == NSControlStateValueOn);
...
- (IBAction)updateUserList:(id)sender
{
	...
	gLastTimeOfEntrySent = [NSDate date];
}
...
- (id)tableView:(NSTableView*)tableView objectValueForTableColumn:(NSTableColumn*)tableColumn row:(NSInteger)rowIndex
{
	if (tableView == self.userTable) {
		...
		NSFileWrapper* fw = [[NSFileWrapper alloc] initRegularFileWithContents:[NSData data]];
		...
		NSMutableAttributedString* val = [[NSMutableAttributedString alloc] initWithString:attach.name];
...
- (void)windowWillClose:(NSNotification*)aNotification
{
	...
	// ARC自己保持解除：ウィンドウクローズ時に強参照を手放す
	@synchronized([SendControl class]) {
		[gOpenSendWindows removeObject:self];
	}
}
...
- (void)awakeFromNib
{
	...
	// 検索フィールドのメニュー設定
    [self.searchMenu itemWithTag:_SEARCH_MENUITEM_TAG_USER].state	= config.sendSearchByUserName ? NSControlStateValueOn : NSControlStateValueOff;
    [self.searchMenu itemWithTag:_SEARCH_MENUITEM_TAG_GROUP].state	= config.sendSearchByGroupName ? NSControlStateValueOn : NSControlStateValueOff;
    [self.searchMenu itemWithTag:_SEARCH_MENUITEM_TAG_HOST].state	= config.sendSearchByHostName ? NSControlStateValueOn : NSControlStateValueOff;
    [self.searchMenu itemWithTag:_SEARCH_MENUITEM_TAG_LOGON].state	= config.sendSearchByLogOnName ? NSControlStateValueOn : NSControlStateValueOff;
