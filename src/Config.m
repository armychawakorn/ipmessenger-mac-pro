...
@property(strong)	NSMutableArray<NSString*>*		broadcastHostList;
@property(strong)	NSMutableArray<NSString*>*		broadcastIPList;
@property(strong)	NSFont*							defaultMessageFont;
@property(strong)	NSArray<NSDictionary*>*			defaultAbsences;
@property(strong)	NSMutableArray<NSDictionary*>*	absenceList;
@property(strong)	NSSound*						receiveSound;
@property(strong)	NSMutableArray<RefuseInfo*>*	refuseList;
...
- (id)init
{
	...
	// フォントのデフォルト値
	_defaultMessageFont = [NSFont systemFontOfSize:[NSFont systemFontSize]];
	...
	if (str && (fVal > 0)) {
		sendWindowMessageFont	= [NSFont fontWithName:str size:fVal];
	}
	// 受信
	str							= [defaults stringForKey:RECV_SOUND];
	_receiveSound				= [NSSound soundNamed:str];
	...
	if (str && (fVal > 0)) {
		recvWindowMessageFont	= [NSFont fontWithName:str size:fVal];
	}
	// 不在
	...
	_refuseList					= [self convertRefuseDefaultsToInfo:array];
	// ログ
...
- (NSString*)broadcastAtIndex:(NSUInteger)index
{
	@try {
		...
		@synchronized (self.broadcastIPList) {
			NSUInteger ipIndex = index - hostnum;
			if (ipIndex < self.broadcastIPList.count) {
				return self.broadcastIPList[ipIndex];
			} else {
				ERR(@"Index out of range (index=%lu, hostnum=%lu, ipListCount=%lu)", 
					(unsigned long)index, (unsigned long)hostnum, (unsigned long)self.broadcastIPList.count);
			}
		}
...
- (void)removeBroadcastAtIndex:(NSUInteger)index
{
	@try {
		...
		@synchronized (self.broadcastIPList) {
			NSUInteger ipIndex = index - hostnum;
			if (ipIndex < self.broadcastIPList.count) {
				[self.broadcastIPList removeObjectAtIndex:ipIndex];
			} else {
				ERR(@"Index out of range (index=%lu, hostnum=%lu, ipListCount=%lu)", 
					(unsigned long)index, (unsigned long)hostnum, (unsigned long)self.broadcastIPList.count);
			}
...
{
	// Sparkle 2: AppDelegateからupdaterを取得
	id appDelegate = NSApp.delegate;
	if ([appDelegate respondsToSelector:@selector(updaterController)]) {
		SPUStandardUpdaterController *updaterController = [appDelegate valueForKey:@"updaterController"];
...
- (void)setUpdateAutomaticCheck:(BOOL)b
{
	...
	if ([appDelegate respondsToSelector:@selector(updaterController)]) {
		SPUStandardUpdaterController *updaterController = [appDelegate valueForKey:@"updaterController"];
		updaterController.updater.automaticallyChecksForUpdates = b;
	}
}
...
- (void)upAbsenceAtIndex:(NSUInteger)index
{
	@try {
		@synchronized (self.absenceList) {
			NSDictionary* obj = self.absenceList[index];
...
- (void)upRefuseInfoAtIndex:(NSUInteger)index
{
	@try {
		@synchronized (self.refuseList) {
			RefuseInfo* obj = self.refuseList[index];
