...
@property(strong)	NSArray<RecvAttachment*>*	attachments;
@property(assign)	NSInteger					packetNo;
@property(strong)	UserInfo*					fromUser;
...
@property(strong)	NSLock*			udpServerLock;		// UDPサーバ待ち合わせ用ロック
@property(assign)	BOOL			udpServerStop;		// UDPサーバ停止フラグ
@property(strong)	_SendList*		sendList;			// 応答待ちメッセージ一覧（再送用）

// 添付ファイル送受信関連
@property(strong)	_AttachList*	attachList;			// 送信添付ファイル一覧
@property			int				tcpSocket;			// TCPソケットディスクリプタ
@property(strong)	NSLock*			tcpServerLock;		// サーバスレッド終了同期用ロック
...
- (BOOL)startupServer
{
	...
	if ([self.udpServerLock tryLock]) {
		...
		while (bind(_udpSocket, (struct sockaddr*)&addr, sizeof(addr)) < 0) {
			...
			(void)[[PortChangeControl alloc] init];
			self.portNo	= Config.sharedConfig.portNo;
			addr.sin_port = htons(self.portNo);
		}
...
- (void)sendMessage:(SendMessage*)msg to:(NSArray<UserInfo*>*)toUsers
{
	...
	for (UserInfo* user in toUsers) {
		...
		if (pNo >= 0) {
			...
			// リトライタイマ発行
			retry.retryTimer = [NSTimer scheduledTimerWithTimeInterval:RETRY_INTERVAL
																 target:self
...
- (void)retryMessage:(NSTimer*)timer
{
	...
	if (retryInfo) {
		if (retryInfo.retryCount >= RETRY_MAX) {
			...
			// タイマ再発行
			retryInfo.retryTimer = [NSTimer scheduledTimerWithTimeInterval:RETRY_INTERVAL
																	 target:self
																   selector:@selector(retryMessage:)
							   ...
							   from:(struct sockaddr_in)fromAddr
{
	...
	case IPMSG_BR_ABSENCE: {
		...
		if (!isUnknownUser) {
			...
			UserInfo* prevUser = fromUser;
			fromUser = [UserInfo userWithHostName:hostName
										logOnName:logOnUser
										  address:&fromAddr];
			// 取得済みの公開鍵・暗号化能力は作り直したユーザ情報へ引き継ぐ
			// （引き継がないと再送待ち中に保持していた鍵が失われ、
			// 　暗号化対応相手への送信がGETPUBKEYを繰り返してリトライアウトする）
			fromUser.publicKey			= prevUser.publicKey;
			fromUser.cryptoCapability	= prevUser.cryptoCapability;
		}
	...
	case IPMSG_SENDMSG: {	// メッセージ送信パケット
		...
		// 受信メッセージ情報構築
		RecvMessage* recvMsg = [[RecvMessage alloc] init];
	...
	case IPMSG_RECVMSG: {	// メッセージ受信確認パケット
		...
		[self.sendList[key].retryTimer invalidate];
	...
	case IPMSG_READMSG: {	// 封書開封通知パケット
		...
		if (config.noticeSealOpened) {
			...
			dispatch_async(dispatch_get_main_queue(), ^{
				(void)[[NoticeControl alloc] initWithTitle:NSLocalizedString(@"SealOpenDlg.title", nil)
	...
	case IPMSG_SENDABSENCEINFO: {
		...
		// 不在情報をダイアログに出す
		dispatch_async(dispatch_get_main_queue(), ^{
			(void)[[NoticeControl alloc] initWithTitle:fromUser.summaryString
										 message:appendix
											date:nil];
		});
		break;
		...
		if (fromUser.publicKey) {
			...
			for (RetryInfo* retryInfo in self.sendList.allValues) {
				if ([retryInfo.toUser isEqual:fromUser]) {
					// 不在解除等でユーザ情報が作り直されると、再送待ちが保持する
					// UserInfoと公開鍵を受信したUserInfoが別インスタンスになり得る。
					// 受信した公開鍵・暗号化能力を再送待ち側にも反映してから送信する。
					// （反映しないとsendTo:で公開鍵未取得と判定され送信できず、
					// 　GETPUBKEYを繰り返してリトライアウトする。特に自分自身宛で発生）
					if (retryInfo.toUser != fromUser) {
						retryInfo.toUser.publicKey			= fromUser.publicKey;
						retryInfo.toUser.cryptoCapability	= fromUser.cryptoCapability;
					}
...
- (void)parseReceivedAttachments:(NSString*)attachMessage to:(RecvMessage*)recvMsg
{
	...
	if (attachList.count > 0) {
		for (__strong NSString* attachStr in attachList) {
...
- (RecvAttachment*)parseAttachmentBuffer:(NSString*)buf needReadModTime:(BOOL)flag
{
	...
	@try {
		...
		// ファイル名部分を切り出す
		NSString* nameEscaped = [buf substringToIndex:range.location];
	...
	case IPMSG_FILE_CLIPBOARD:
		attach = [[RecvClipboard alloc] init];
...
- (BOOL)parseAnsPubkey:(NSString*)appendix from:(UserInfo*)fromUser
{
	...
	CryptoCapability* cap = [[CryptoCapability alloc] init];
